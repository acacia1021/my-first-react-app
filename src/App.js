...
// if a function accepts a parameter that will contain an object it 
// can be destructured to "pull out" the object properties and make them 
// available as locally scoped variables (i.e., variables only available 
...
function storeOrder2({id2, currency2}) { // destructuring
  localStorage.setItem('id2', id2);
  localStorage.setItem('currency2', currency2);
}

// The destructuring syntax is the same as taught in the previous 
// lecture - just without creating a constant or variable manually.

// Instead, id and currency are "pulled out" of the incoming object 
// (i.e., the object passed as an argument to storeOrder).

// It's very important to understand, that storeOrder still only 
// takes one parameter in this example! It does not accept two 
// parameters. Instead, it's one single parameter - an object which 
// then just is destructured internally.

// The function would still be called like this:

storeOrder2({id: 5, currency: 'USD', amount: 15.99}); 
// which is one argument / value!

// console.log(localStorage.getItem(storeOrder.id));
